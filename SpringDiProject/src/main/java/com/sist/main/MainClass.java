 ...
 *  스프링 컨테이너 동작
 *  1) 스프링 컨테이너 생성
 *  	ApplicationContext app = new ClassPatchXmlApplicationContext("app.xml")
 *  2) Bean 생성 (클래스 객체 생성)
 ...
 *  	b	new B()
 *  	------------
 *  	c	new C() => getBeand("c") => new C()
 *  	------------
 *  	*** 모든 객체는 싱글턴을 이용한다 (저장된 객체 주소가 변경이 안된다)
 *  	*** getBean()을 호출할 때마다 => 객체 생성이 가능
 *  		scope="singleton" scope="prototype"
 *  3) DI 수행 => setter, 생성자를 이용한 데이터 주입
 *  4) 요청 메소드 호출
 *  	= 객체 생성시 => init-method
 *  	= 객체 소멸시 => distroy-method
 *  5) 개발자가 객체 활용
